...
# Example:
#
#    lwp_parser.pl http://www.cs.jhu.edu/~jkloss/index.shtml
#
...
if ( $response->content_type =~ m@text/html@ ){
    print "HTML doc!\n";
} elsif ( $response->content_type =~ m@image@ ){
    print "Image!\n";
} elsif ( $response->content_type =~ m@application/pdf@ ){
    print "PDF!\n";
}
...
foreach my $item (@{ $html_tree->extract_links( "a" )}) {
    ...
    my $baseurl = URI->new( $response->base );
    my $basedomain = $baseurl->host;

	if (!( $domain =~ $basedomain )) { # if domains don't match, link is non-local
		print "Non-local: $furl \n";
	} elsif ($link !~ /\/#/) {
        print "Not Self-ref: $furl \n";
    }
