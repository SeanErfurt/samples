/** DomainTally
 *
 *      A program which extracts and counts occurrences of domains in a
 *      file like urls.txt. When it encounters the string "report" in
 *      the file, it outputs the current most common domain name.
 *      Uses a Maximum Priority Queue based on the Sedgewick implementation
 *      for keeping track of the most common domain name.
 *
...
public class DomainTally {
    ...
    public static void main(String[] args) {
        ...
        String regex = "(?:((ht|f)tp(s?)\\:\\/\\/)?(www.)?)" + doms[i]
                + "(com|edu|org|net|gov)";
        // read in arguments
        for (String s : args) {
            try {
                ...
                try {
                    ...
                    while ((line = reader.readLine()) != null) {
                        String[] ra = line.split("\\s+"); // remove whitespace
                        for (String word : ra) {
                            if (word.equals("report")) {
                                pq.max(); // undef
                            } else {
                                // register new domain
                                pq.insert(word);
        ...
        while (stdin.hasNext()) {
            ...
            if (url.equals("report")) {
                pq.max(); // undef
            } else {
                Pattern.compile(
                        "(?:((ht|f)tp(s?)\\:\\/\\/)?(www.)?)" + "(\\w+)"
                                + "(com|edu|org|net|gov)").matcher(url);
                // register url group or find existing
    ...
    private class MaxPQ<Key> implements Iterable<Key> {
         ...
         * in descending order. The iterator doesn't implement <tt>remove()</tt>
         * since it's optional.
         *
